...
typedef ReadL_rtModel RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * ReadL(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(ReadL_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://ReadL:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: ReadL -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
