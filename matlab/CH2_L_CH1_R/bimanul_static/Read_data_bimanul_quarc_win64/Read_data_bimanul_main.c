...
typedef Read_data_bimanul_rtModel RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Read_data_bimanul(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Read_data_bimanul_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Read_data_bimanul -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
