...
typedef Read0_rtModel RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Read0(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Read0_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://Read0:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Read0 -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
