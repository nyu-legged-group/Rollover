...
typedef Read_data_optimal_rtModel RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Read_data_optimal(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Read_data_optimal_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Read_data_optimal -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
