...
void Read_data_optimal_output(int_T tid)
{
    ...
    if (fp != (NULL)) {
      ...
      if (fwrite(u, sizeof(real_T), 2, fp) != 2) {
        rtmSetErrorStatus(Read_data_optimal_M,
                          "Error writing to MAT-file 9_17_Rollover001_ch3.mat");
        return;
      }
      ...
      if (((++Read_data_optimal_DWork.ToFile_IWORK.Count)*2)+1 >= 100000000) {
        (void)fprintf(stdout,
                      ...
                      "    allowed in MAT-file 9_17_Rollover001_ch3.mat.\n");
      }
    }
  }
  ...
  if (!(++Read_data_optimal_DWork.ToFile1_IWORK.Decimation % 1) &&
    ...
    if (fp != (NULL)) {
      ...
      if (fwrite(u, sizeof(real_T), 2, fp) != 2) {
        rtmSetErrorStatus(Read_data_optimal_M,
                          "Error writing to MAT-file 9_17_Rollover001_ch2.mat");
        return;
      }
      ...
      if (((++Read_data_optimal_DWork.ToFile1_IWORK.Count)*2)+1 >= 100000000) {
        (void)fprintf(stdout,
                      ...
                      "    the simulation has ended, because it has reached\n"
                      "    the maximum number of elements (100000000)\n"
                      "    allowed in MAT-file 9_17_Rollover001_ch2.mat.\n");
      }
    }
  }
...
void Read_data_optimal_initialize(boolean_T firstTime)
{
  ...
  /* external mode info */
  Read_data_optimal_M->Sizes.checksums[0] = (1464505566U);
  Read_data_optimal_M->Sizes.checksums[1] = (643891978U);
  Read_data_optimal_M->Sizes.checksums[2] = (3156274655U);
  Read_data_optimal_M->Sizes.checksums[3] = (166446009U);
...
void Read_data_optimal_terminate(void)
{
  ...
  /* Terminate for ToFile: '<Root>/To File' */
  {
    FILE *fp = (FILE *) Read_data_optimal_DWork.ToFile_PWORK.FilePtr;
    if (fp != (NULL)) {
      char fileName[509] = "9_17_Rollover001_ch3.mat";
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(Read_data_optimal_M,
                          "Error closing MAT-file 9_17_Rollover001_ch3.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(Read_data_optimal_M,
                          "Error reopening MAT-file 9_17_Rollover001_ch3.mat");
        return;
      }
      ...
      if (rt_WriteMat4FileHeader(fp, 2,
        ...
        rtmSetErrorStatus(Read_data_optimal_M,
                          "Error writing header for ch0 to MAT-file 9_17_Rollover001_ch3.mat");
      }
  ...
  /* Terminate for ToFile: '<Root>/To File1' */
  {
    FILE *fp = (FILE *) Read_data_optimal_DWork.ToFile1_PWORK.FilePtr;
    if (fp != (NULL)) {
      char fileName[509] = "9_17_Rollover001_ch2.mat";
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(Read_data_optimal_M,
                          "Error closing MAT-file 9_17_Rollover001_ch2.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(Read_data_optimal_M,
                          "Error reopening MAT-file 9_17_Rollover001_ch2.mat");
        return;
      }
      ...
      if (rt_WriteMat4FileHeader(fp, 2,
        ...
        rtmSetErrorStatus(Read_data_optimal_M,
                          "Error writing header for ch1 to MAT-file 9_17_Rollover001_ch2.mat");
      }
...
void MdlStart(void)
{
  ...
  /* Start for ToFile: '<Root>/To File' */
  {
    ...
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(Read_data_optimal_M,
                        "Error creating .mat file 9_17_Rollover001_ch3.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp,2,0,"ch0")) {
      rtmSetErrorStatus(Read_data_optimal_M,
                        "Error writing mat file header to file 9_17_Rollover001_ch3.mat");
      return;
    }
  ...
  /* Start for ToFile: '<Root>/To File1' */
  {
    ...
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(Read_data_optimal_M,
                        "Error creating .mat file 9_17_Rollover001_ch2.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp,2,0,"ch1")) {
      rtmSetErrorStatus(Read_data_optimal_M,
                        "Error writing mat file header to file 9_17_Rollover001_ch2.mat");
      return;
    }
