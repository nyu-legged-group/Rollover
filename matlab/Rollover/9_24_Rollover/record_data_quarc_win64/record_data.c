    ...
    if (fp != (NULL)) {
      ...
      if (fwrite(u, sizeof(real_T), 2, fp) != 2) {
        rtmSetErrorStatus(record_data_M,
                          "Error writing to MAT-file 9_24_Rollover008_right_sensor.mat");
        return;
      }
      ...
      if (((++record_data_DWork.ToFile_IWORK.Count)*2)+1 >= 100000000) {
        (void)fprintf(stdout,
                      ...
                      "    allowed in MAT-file 9_24_Rollover008_right_sensor.mat.\n");
      }
    }
  }
...
void record_data_initialize(boolean_T firstTime)
{
  ...
  /* external mode info */
  record_data_M->Sizes.checksums[0] = (3125857871U);
  record_data_M->Sizes.checksums[1] = (3337065614U);
  record_data_M->Sizes.checksums[2] = (42826559U);
  record_data_M->Sizes.checksums[3] = (768167036U);
...
void record_data_terminate(void)
{
  ...
  /* Terminate for ToFile: '<Root>/To File' */
  {
    FILE *fp = (FILE *) record_data_DWork.ToFile_PWORK.FilePtr;
    if (fp != (NULL)) {
      char fileName[509] = "9_24_Rollover008_right_sensor.mat";
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(record_data_M,
                          "Error closing MAT-file 9_24_Rollover008_right_sensor.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(record_data_M,
                          "Error reopening MAT-file 9_24_Rollover008_right_sensor.mat");
        return;
      }
      ...
      if (rt_WriteMat4FileHeader(fp, 2, record_data_DWork.ToFile_IWORK.Count,
           "ch0")) {
        rtmSetErrorStatus(record_data_M,
                          "Error writing header for ch0 to MAT-file 9_24_Rollover008_right_sensor.mat");
      }
...
void MdlStart(void)
{
  ...
  /* Start for ToFile: '<Root>/To File' */
  {
    ...
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(record_data_M,
                        "Error creating .mat file 9_24_Rollover008_right_sensor.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp,2,0,"ch0")) {
      rtmSetErrorStatus(record_data_M,
                        "Error writing mat file header to file 9_24_Rollover008_right_sensor.mat");
      return;
    }
