...
typedef record_data_rtModel RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * record_data(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(record_data_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://record_data:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: record_data -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
