...
typedef Read_data_Test_rtModel RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Read_data_Test(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Read_data_Test_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://Read_data_Test:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Read_data_Test -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
