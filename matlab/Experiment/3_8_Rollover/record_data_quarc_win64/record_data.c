    ...
    if (fp != (NULL)) {
      ...
      if (fwrite(u, sizeof(real_T), 2, fp) != 2) {
        rtmSetErrorStatus(record_data_M,
                          "Error writing to MAT-file 0325Rollover03_right_sensor.mat");
        return;
      }
      ...
      if (((++record_data_DWork.ToFile_IWORK.Count)*2)+1 >= 100000000) {
        (void)fprintf(stdout,
                      ...
                      "    allowed in MAT-file 0325Rollover03_right_sensor.mat.\n");
      }
    }
  }
...
void record_data_initialize(boolean_T firstTime)
{
  ...
  /* external mode info */
  record_data_M->Sizes.checksums[0] = (641744073U);
  record_data_M->Sizes.checksums[1] = (1975221608U);
  record_data_M->Sizes.checksums[2] = (1361638603U);
  record_data_M->Sizes.checksums[3] = (2690163821U);
...
void record_data_terminate(void)
{
  ...
  /* Terminate for ToFile: '<Root>/To File' */
  {
    FILE *fp = (FILE *) record_data_DWork.ToFile_PWORK.FilePtr;
    if (fp != (NULL)) {
      char fileName[509] = "0325Rollover03_right_sensor.mat";
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(record_data_M,
                          "Error closing MAT-file 0325Rollover03_right_sensor.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(record_data_M,
                          "Error reopening MAT-file 0325Rollover03_right_sensor.mat");
        return;
      }
      ...
      if (rt_WriteMat4FileHeader(fp, 2, record_data_DWork.ToFile_IWORK.Count,
           "rRollover")) {
        rtmSetErrorStatus(record_data_M,
                          "Error writing header for rRollover to MAT-file 0325Rollover03_right_sensor.mat");
      }
...
void MdlStart(void)
{
  ...
  /* Start for ToFile: '<Root>/To File' */
  {
    ...
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(record_data_M,
                        "Error creating .mat file 0325Rollover03_right_sensor.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp,2,0,"rRollover")) {
      rtmSetErrorStatus(record_data_M,
                        "Error writing mat file header to file 0325Rollover03_right_sensor.mat");
      return;
    }
