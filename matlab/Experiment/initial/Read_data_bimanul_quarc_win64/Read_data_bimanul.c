 ...
 * C source code generated on : Mon Mar 08 17:30:01 2021
 *
...
void Read_data_bimanul_output(int_T tid)
{
    ...
    if (fp != (NULL)) {
      ...
      if (fwrite(u, sizeof(real_T), 2, fp) != 2) {
        rtmSetErrorStatus(Read_data_bimanul_M,
                          "Error writing to MAT-file 3_8_Optimal001_Right.mat");
        return;
      }
      ...
      if (((++Read_data_bimanul_DWork.ToFile_IWORK.Count)*2)+1 >= 100000000) {
        (void)fprintf(stdout,
                      ...
                      "    allowed in MAT-file 3_8_Optimal001_Right.mat.\n");
      }
    }
  }
  ...
  if (!(++Read_data_bimanul_DWork.ToFile1_IWORK.Decimation % 1) &&
    ...
    if (fp != (NULL)) {
      ...
      if (fwrite(u, sizeof(real_T), 2, fp) != 2) {
        rtmSetErrorStatus(Read_data_bimanul_M,
                          "Error writing to MAT-file 3_8_Optimal001_Left.mat");
        return;
      }
      ...
      if (((++Read_data_bimanul_DWork.ToFile1_IWORK.Count)*2)+1 >= 100000000) {
        (void)fprintf(stdout,
                      ...
                      "    the simulation has ended, because it has reached\n"
                      "    the maximum number of elements (100000000)\n"
                      "    allowed in MAT-file 3_8_Optimal001_Left.mat.\n");
      }
    }
  }
...
void Read_data_bimanul_initialize(boolean_T firstTime)
{
  ...
  /* external mode info */
  Read_data_bimanul_M->Sizes.checksums[0] = (722601396U);
  Read_data_bimanul_M->Sizes.checksums[1] = (3524655249U);
  Read_data_bimanul_M->Sizes.checksums[2] = (3669835955U);
  Read_data_bimanul_M->Sizes.checksums[3] = (3542451934U);
...
void Read_data_bimanul_terminate(void)
{
  ...
  /* Terminate for ToFile: '<Root>/To File' */
  {
    FILE *fp = (FILE *) Read_data_bimanul_DWork.ToFile_PWORK.FilePtr;
    if (fp != (NULL)) {
      char fileName[509] = "3_8_Optimal001_Right.mat";
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(Read_data_bimanul_M,
                          "Error closing MAT-file 3_8_Optimal001_Right.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(Read_data_bimanul_M,
                          "Error reopening MAT-file 3_8_Optimal001_Right.mat");
        return;
      }
      ...
      if (rt_WriteMat4FileHeader(fp, 2,
        ...
        rtmSetErrorStatus(Read_data_bimanul_M,
                          "Error writing header for R to MAT-file 3_8_Optimal001_Right.mat");
      }
  ...
  /* Terminate for ToFile: '<Root>/To File1' */
  {
    FILE *fp = (FILE *) Read_data_bimanul_DWork.ToFile1_PWORK.FilePtr;
    if (fp != (NULL)) {
      char fileName[509] = "3_8_Optimal001_Left.mat";
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(Read_data_bimanul_M,
                          "Error closing MAT-file 3_8_Optimal001_Left.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(Read_data_bimanul_M,
                          "Error reopening MAT-file 3_8_Optimal001_Left.mat");
        return;
      }
      ...
      if (rt_WriteMat4FileHeader(fp, 2,
        ...
        rtmSetErrorStatus(Read_data_bimanul_M,
                          "Error writing header for L to MAT-file 3_8_Optimal001_Left.mat");
      }
...
void MdlStart(void)
{
  ...
  /* Start for ToFile: '<Root>/To File' */
  {
    ...
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(Read_data_bimanul_M,
                        "Error creating .mat file 3_8_Optimal001_Right.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp,2,0,"R")) {
      rtmSetErrorStatus(Read_data_bimanul_M,
                        "Error writing mat file header to file 3_8_Optimal001_Right.mat");
      return;
    }
  ...
  /* Start for ToFile: '<Root>/To File1' */
  {
    ...
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(Read_data_bimanul_M,
                        "Error creating .mat file 3_8_Optimal001_Left.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp,2,0,"L")) {
      rtmSetErrorStatus(Read_data_bimanul_M,
                        "Error writing mat file header to file 3_8_Optimal001_Left.mat");
      return;
    }
