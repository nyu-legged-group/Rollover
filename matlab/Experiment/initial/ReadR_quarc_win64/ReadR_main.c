...
typedef ReadR_rtModel RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * ReadR(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(ReadR_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://ReadR:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: ReadR -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
