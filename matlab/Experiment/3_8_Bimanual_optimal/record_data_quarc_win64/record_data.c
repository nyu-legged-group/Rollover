    ...
    if (fp != (NULL)) {
      ...
      if (fwrite(u, sizeof(real_T), 2, fp) != 2) {
        rtmSetErrorStatus(record_data_M,
                          "Error writing to MAT-file 3_8_Rollover01_right_sensor.mat");
        return;
      }
      ...
      if (((++record_data_DWork.ToFile_IWORK.Count)*2)+1 >= 100000000) {
        (void)fprintf(stdout,
                      ...
                      "    allowed in MAT-file 3_8_Rollover01_right_sensor.mat.\n");
      }
    }
  }
...
void record_data_initialize(boolean_T firstTime)
{
  ...
  rtmSetTFinal(record_data_M, 2.0);
  record_data_M->Timing.stepSize0 = 0.001;

  /* external mode info */
  record_data_M->Sizes.checksums[0] = (3264938521U);
  record_data_M->Sizes.checksums[1] = (244649415U);
  record_data_M->Sizes.checksums[2] = (843081022U);
  record_data_M->Sizes.checksums[3] = (3274995015U);
...
void record_data_terminate(void)
{
  ...
  /* Terminate for ToFile: '<Root>/To File' */
  {
    FILE *fp = (FILE *) record_data_DWork.ToFile_PWORK.FilePtr;
    if (fp != (NULL)) {
      char fileName[509] = "3_8_Rollover01_right_sensor.mat";
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(record_data_M,
                          "Error closing MAT-file 3_8_Rollover01_right_sensor.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(record_data_M,
                          "Error reopening MAT-file 3_8_Rollover01_right_sensor.mat");
        return;
      }
      ...
      if (rt_WriteMat4FileHeader(fp, 2, record_data_DWork.ToFile_IWORK.Count,
           "rRollover")) {
        rtmSetErrorStatus(record_data_M,
                          "Error writing header for rRollover to MAT-file 3_8_Rollover01_right_sensor.mat");
      }
...
void MdlStart(void)
{
  ...
  /* Start for ToFile: '<Root>/To File' */
  {
    ...
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(record_data_M,
                        "Error creating .mat file 3_8_Rollover01_right_sensor.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp,2,0,"rRollover")) {
      rtmSetErrorStatus(record_data_M,
                        "Error writing mat file header to file 3_8_Rollover01_right_sensor.mat");
      return;
    }
